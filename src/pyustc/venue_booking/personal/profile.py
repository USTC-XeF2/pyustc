...
async def order_result(client: USTCSportClient, order_no: str) -> dict:
    """查询订单结果"""
    return await client.get("/order/result/", {"order_no": order_no})


async def my_current(client: USTCSportClient) -> dict:
    """个人信息"""
    return await client.get("/my/current")


async def friend_list(client: USTCSportClient) -> dict:
    """好友列表 + 待处理请求"""
    return await client.get("/friend")


async def friend_search(client: USTCSportClient, keyword: str) -> dict:
    """搜索用户(按昵称/学号搜索可添加的好友)"""
    return await client.get("/friend/search", {"keyword": keyword, "scene": "friend"})


async def friend_request(client: USTCSportClient, player: dict) -> dict:
...
async def friend_delete(client: USTCSportClient, friend_id: int) -> dict:
    """删除好友(friend_id 在 URL 路径中)"""
    return await client.post(f"/friend/delete/{friend_id}")


async def overflow(client: USTCSportClient) -> dict:
    """违约记录"""
    return await client.get("/overflow")


async def punish(client: USTCSportClient, kind_id: int = 0, page: int = 1) -> dict:
    """惩罚/违规详情"""
    return await client.get(f"/punish/{kind_id}", {"kind_id": kind_id, "page": page})


async def chart(client: USTCSportClient, tag: str = "", scene: str = "month") -> dict:
    """统计数据"""
